...
import 'package:chatup/chat/model/message.dart';
...
class AppService extends ChangeNotifier {
  ...
  Future<void> _createUser(int i) async {
    final AuthResponse response = await _supabase.auth.signUp(email: 'test_$i@test.com', password: _password);
  ...
  String getCurrentUserId() => isAuthentificated() ? _supabase.auth.currentUser!.id : '';


  String getCurrentUserEmail() => isAuthentificated() ? _supabase.auth.currentUser!.email ?? '' : '';
...
class ItemModel {
  ...
  ItemModel({required this.title, required this.icon, this.function});
}
